...
async function init() {
  ...
  this.hide('#main');
  await loadFiles();
  problemsToShow = resultsData.allProblems;
  updateFilterValues();
  updateProblemsView();
  this.hide('#spinner');
  this.show('#main');
...
function addFilter(htmlElement) {
  problemsLimit = 20;
  let type = htmlElement.dataset.type;
  const value = htmlElement.dataset.value;
  if (type === 'type') {
    if (typeFilter.includes(value)) {
      typeFilter = typeFilter.filter((item) => item !== value);
    } else {
      typeFilter.push(value);
    }
  } else if (type === 'severity') {
    const numValue = parseInt(value);
    if (severityFilter.includes(numValue)) {
      severityFilter = severityFilter.filter((item) => item !== numValue);
    } else {
      severityFilter.push(numValue);
    }
  } else if (type === 'language') {
    if (languageFilter.includes(value)) {
      languageFilter = languageFilter.filter((item) => item !== value);
    } else {
      languageFilter.push(value);
    }
  } else if (type === 'rule') {
    if (ruleFilter.includes(value)) {
      ruleFilter = ruleFilter.filter((item) => item !== value);
    } else {
      ruleFilter.push(value);
    }
  } else if (type === 'file') {
    if (fileFilter.includes(value)) {
      fileFilter = fileFilter.filter((item) => item !== value);
    } else {
...
function updateFilterValues(filterBy) {
  ...
  for (const data of resultsData.allProblems) {
    const type = data.ruleType ?? 'Uncatalogued';
    if (
    ...
    if (fileSearchTerm && !data.path.toLowerCase().includes(fileSearchTerm.toLowerCase())) {
      continue;
    }
    if (ruleSearchTerm && !data.ruleName.toLowerCase().includes(ruleSearchTerm.toLowerCase())) {
      continue;
    }
    problemsToShow.push(data);
  }

  for (const data of problemsToShow) {
    const type = data.ruleType ?? 'Uncatalogued';
    if (!issuesByType[type]) {
      issuesByType[type] = 0;
    }
    issuesByType[type]++;
    if (!issuesBySeverity[data.severity]) {
      issuesBySeverity[data.severity] = 0;
    }
    issuesBySeverity[data.severity]++;
    if (!issuesByLanguage[data.language]) {
      issuesByLanguage[data.language] = 0;
    }
    issuesByLanguage[data.language]++;
    if (!issuesByRule[data.ruleName]) {
      issuesByRule[data.ruleName] = 0;
    }
    issuesByRule[data.ruleName]++;
    if (!issuesByFile[data.path]) {
      issuesByFile[data.path] = 0;
    }
    issuesByFile[data.path]++;
    if (!problemsByFile[data.path]) {
  ...
  if (problemsLimit > Object.keys(problemsByFile).length) {
    problemsLimit = Object.keys(problemsByFile).length;
  }

  const bugs = issuesByType['Bug'] ?? 0;
  const vulnerabilities = issuesByType['Vulnerability'] ?? 0;
  const securityHotspots = issuesByType['Security Hotspot'] ?? 0;
  const codeSmells = issuesByType['Code Smell'] ?? 0;
  const uncatalogued = issuesByType['Uncatalogued'] ?? 0;

  const blockers = issuesBySeverity[1] ?? 0;
  const criticals = issuesBySeverity[2] ?? 0;
  const majors = issuesBySeverity[3] ?? 0;
  ...
  setHtml('#codeSmellFilterValue', codeSmells);
  setHtml('#uncataloguedFilterValue', uncatalogued);

  setHtml('#blockersFilterValue', blockers);
  setHtml('#criticalsFilterValue', criticals);
  ...
  if (!bugs) {
    ...
    enable('#bugFilterElement');
  }
  if (!vulnerabilities) {
    disable('#vulnerabilityFilterElement');
  } else {
    enable('#vulnerabilityFilterElement');
  }
  if (!securityHotspots) {
    disable('#securityHotspotFilterElement');
  } else {
    enable('#securityHotspotFilterElement');
  }
  if (!codeSmells) {
    disable('#codeSmellFilterElement');
  } else {
    enable('#codeSmellFilterElement');
  }
  if (!uncatalogued) {
    disable('#uncataloguedFilterElement');
  } else {
    enable('#uncataloguedFilterElement');
  }

  if (!blockers) {
    disable('#blockerFilterElement');
  } else {
    enable('#blockerFilterElement');
  }
  if (!criticals) {
    disable('#criticalFilterElement');
  } else {
    enable('#criticalFilterElement');
  }
  if (!majors) {
    disable('#majorFilterElement');
  } else {
    enable('#majorFilterElement');
  }
  if (!minors) {
    disable('#minorFilterElement');
  } else {
    enable('#minorFilterElement');
  }
  if (!infos) {
    disable('#infoFilterElement');
  } else {
  ...
  if (typeFilter.length > 0) {
    ...
    hide('#issuesByTypeBadge');
  }
  if (severityFilter.length > 0) {
    setHtml('#issuesBySeverityBadge', severityFilter.map((item) => SEVERITY[item]).join(', '));
    show('#issuesBySeverityBadge');
  } else {
    hide('#issuesBySeverityBadge');
  }

  if (languageFilter.length > 0) {
    setHtml('#issuesByLanguageBadge', languageFilter.join(', '));
    show('#issuesByLanguageBadge');
  } else {
    hide('#issuesByLanguageBadge');
  }

  if (ruleFilter.length > 0) {
    setHtml('#issuesByRuleBadge', ruleFilter.join(', '));
    show('#issuesByRuleBadge');
  } else {
    hide('#issuesByRuleBadge');
  }

  if (fileFilter.length > 0) {
    setHtml('#issuesByFileBadge', fileFilter.join(', '));
  ...
  setHtml(
    '#issuesByLanguageFiltersContent',
    Object.keys(issuesByLanguage)
      .map((key) => {
        const data = issuesByLanguage[key];
        return getFilterLink('language', key, data);
      })
      .join('')
  );

  setHtml(
    '#issuesByRuleFiltersContent',
    Object.keys(issuesByRule)
      .map((key) => {
        const data = issuesByRule[key];
        return getFilterLink('rule', key, data);
      })
      .join('')
  );

  setHtml(
    '#issuesByFileFiltersContent',
    Object.keys(issuesByFile)
      .map((key) => {
        const root = resultsData.branch ? resultsData.branch : 'project-root';
        const data = issuesByFile[key];
        const fileNametoShow = root + '/force-app' + key.split('force-app')[1].replaceAll('\\', '/');
        return getFilterIconLink(ICONS['file'], 'file', key, data, fileNametoShow);
      })
      .join('')
  );
}

function getFilterLink(category, key, value) {
  return `
  <a href="#" class="row justify-content-between filter-element-link" data-value="${key}" data-type="${category}" onclick="addFilter(this)" title="${key}">
    <div class="col text-truncate">${key}</div>
    <div class="col-auto">${value}</div>
  </a>`;
}

function getFilterIconLink(icon, category, key, value, title) {
  return `
  <a href="#" class="row justify-content-between filter-element-link" data-value="${key}" data-type="${category}" onclick="addFilter(this)" title="${title ? title : key
    }">
    <div class="col text-truncate"><span>${icon}</span> ${title ? title : key}</div>
    <div class="col-auto">${value}</div>
  </a>`;
}

function updateProblemsView() {
  hide('#problemsContent');
  show('#problemsSpinner');
  const html = Object.keys(problemsByFile)
    .slice(0, problemsLimit)
    .map((key) => {
      const data = problemsByFile[key];
      return renderProblemsFile(key, data);
    });
  setHtml('#problemsContent', html.join(''));
  const showMoreContent =
    problemsLimit < Object.keys(problemsByFile).length
      ? `${problemsLimit} of ${Object.keys(problemsByFile).length
      } files shown. <a href="#" onclick="showMore()">Show More</a>`
      : `${problemsLimit} of ${Object.keys(problemsByFile).length} files shown.`;
  setHtml('#problemsContentFilesRendered', showMoreContent);
  hide('#problemsSpinner');
  show('#problemsContent');
}

function renderProblemsFile(file, problems) {
  const root = resultsData.branch ? resultsData.branch : 'project-root';
  const fileNametoShow = root + '/force-app' + file.split('force-app')[1].replaceAll('\\', '/');
  return `
    <div class="text-truncate mt-4 mb-1 ps-3 fw-semibold" style="font-size:13px" title="${fileNametoShow}">${fileNametoShow}</div>
    <ul class="file-problems">
      ${problems.map((problem) => renderProblem(problem)).join('')}
    </ul>
  `;
}
...
function renderProblem(problem) {
  ...
  return `
    <li class="problem">
      <a href="./fileProblems.html?id=${problem.id}" style="text-decoration:none; color: black;">
        ...
        <div class="row px-3 justify-content-between">
          ...
          <div class="col-auto">
            <div class="row justify-content-end">
              ...
              <div class="col-auto"><span>${ICONS[SEVERITY[problem.severity]]}</span> <span style="font-size: 12px">${SEVERITY[problem.severity]
    }</span></div>
            </div>
          </div>
        </div>
...
function changeRuleInputHandler(htmlElement) {
...
function changeFileInputHandler(htmlElement) {
  const value = htmlElement.value;
